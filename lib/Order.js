...
class Order{
    ...
    adminGetOrder(){
        return new Promise((resolve, reject)=>{
            var sql = `select * from mt_order where order_cencel = '0' order by No desc`;
            getConnection((conn)=>{
                conn.query(sql,(err,rows)=>{
                    conn.release();
    ...
    adminOrderState(title,state,page){
        return new Promise((resolve, reject)=>{
            var sql = `update mt_order set ${title} = '${state}' where No = '${Number(page)}'`;
            getConnection((conn)=>{
                conn.query(sql,(err,rows)=>{
                    conn.release();
                    if(err) throw err;
    ...
    adminGetOrderCencel(){
        return new Promise((resolve, reject)=>{
            var sql = `select * from mt_order where order_cencel = '1' order by No desc`;
            getConnection((conn)=>{
                conn.query(sql,(err,rows)=>{
                    conn.release();
                    if(err) return console.log(err);
    ...
    adminGetUserOrder(userId){
        return new Promise((resolve, reject)=>{
            var sql = `select * from mt_order where order_id = '${userId}' order by No desc`;
            getConnection((conn)=>{
                conn.query(sql,(err,rows)=>{
                    conn.release();
                    if(err) return console.log(err);
