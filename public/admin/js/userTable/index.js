var dataGetUrl = "/admin/getUser"; // 데이터를 가져올 링크
...
function successGetData(data){
    ...
    for(var i = 0; i < maxListLength && i < data.length; i++){
        ...
        <tr>
            <td>
              <input type="checkbox" name="check" id="check4" value="${data[i].user_no}">
            </td>
            <td>${data[i].user_name}</td>
            <td>
              <p class="item_name">${data[i].user_id}</p>
            </td>
            <td>${data[i].reg_dt}</td>
            <td>${data[i].user_phone}</td>
            <td>${data[i].user_add}</td>
            <td>${data[i].user_email}</td>
            <td>
              <a href="/admin/userTable/order/?userId=${data[i].user_id}">주문</a>
            </td>
          </tr>
        `
    }
